WITH tb_clientes AS (
    -- Base de clientes ativos (1 linha por cliente)
    SELECT
        cct.codigo AS cod_cliente,
        MIN(DATE(cct.assinatura)) AS primeira_assinatura,

        -- Agregação para o cálculo interno do Score de Atrito (sem vazar para o SELECT final)
    ...
    WHERE cct.assinatura IS NOT NULL
      AND cct.status IN (3, 4) -- Varre o histórico de contratos ativos e cancelados
    GROUP BY cct.codigo
...
-- DATASET DE PRODUÇÃO (APENAS MATRIZ DE RECURSOS EXPLICATIVOS X)
SELECT
    c.cod_cliente,
    c.primeira_assinatura,

    -- FEATURES OPERACIONAIS CONTRATUAIS
    ...
    -- FEATURES DE ESTOQUE DE PENDÊNCIAS EM ABERTO
    ...
    -- RECÊNCIA REAL EM PRODUÇÃO: Avaliação linear baseada no dia de hoje (CURDATE())
    ...
    -- SLA FINANCEIRO (Ponderação pelo faturamento ativo atual da conta)
    ROUND(
        COALESCE(f.media_dias_exec_reclamacao, 0) * COALESCE(c.valor_ativo_total, 0),
        2) AS score_atrito_sla_financeiro,

    -- FEATURE DERIVADA CALCULADA DIRETAMENTE NO BANCO
    -- 30.44 dias para converter em mensal

WITH tb_clientes AS (
    ...
    SELECT
        ...
        -- Se não houver data de cancelamento considers a data de vencimento do contrato
        MAX(
			CASE
				WHEN cct.status = 4
					THEN DATE(
				...
				( -- Comportamento histórico de base do cliente
					(COALESCE(f.qtd_tarefas_total, 0) - COALESCE(f.tarefas_90d, 0)) /
						-- GREATEST(1, ...) é uma blindagem para o caso de um cliente ter entrado hoje
				 		GREATEST(1, DATEDIFF(CURDATE() - INTERVAL 90 DAY, c.primeira_assinatura))), 2
			 )
		ELSE 1
...
-- FILTRO DE PRODUÇÃO
-- Garante a extração exclusiva de clientes vivos na base para scoring preditivo
WHERE c.churn = 0;